...
- **2026-08-26: the 9am Slack morning digest is OFF.** `/api/cron/slack-digest` (13:00 UTC deterministic sales / cash / low-stock / reorder card built by `lib/slack/digest.ts`) is commented out in `vercel.ts` at Paul's request — it posted every day whether or not anything had changed, so it read as stale. Route and digest builder are untouched: re-enable by uncommenting the single cron line and pushing `main`. The 14:00 UTC `/api/cron/po-monitor` LLM briefing (rev 2) is now the only *scheduled* daily Slack post; `slack-triggers` (every 15 min) and `health` (hourly) are unchanged and only fire on real events.
...
- **Shipped July 2026: LLM morning briefing for the PO radar (rev 2 — Paul-approved direction: NO emoji).** The chief-of-staff prompt lives in `lib/slack/briefing.ts` (`BRIEFING_SYSTEM`); `composeBriefing()` runs it on `GLOW_DIGEST_MODEL` (Claude Sonnet) with the fallback chain, fed structured `RadarLine.facts` + vitals dates. Rev 2 structure: *Needs attention* / *Watching* / *On track* roll-up, each entity exactly once in its highest tier; quiet no-correspondence POs with ETAs >2 weeks out demote to On track. Zero emoji — severity reaches the model as words ("action"/"watch"/"ok"), and `sanitizeMrkdwn` strips any emoji that leak. `briefingBlocks()`: `---` → divider blocks, fully-italic lines → dim context blocks. Dated **America/New_York**, date pinned in the prompt (shopify_metrics syncs a UTC day ahead). `/api/cron/po-monitor` falls back to the deterministic blocks if the model is down. **Iterate with `?dry=1`** (composes, no post); **`?test=1`** posts with a throwaway dedupe key + test marker. A plain off-schedule trigger consumes the day's dedupe key and suppresses the scheduled run — don't. Known nits: the Beone 3 run's `product_name` is generic in the record ("manufacturing order" shows in the brief — fix the record, not the prompt).
