...
- **Shipped July 2026: LLM morning briefing for the PO radar.** Paul's chief-of-staff prompt lives verbatim in `lib/slack/briefing.ts` (`BRIEFING_SYSTEM`); `composeBriefing()` runs it on `GLOW_DIGEST_MODEL` (Claude Sonnet) with the fallback chain, fed structured `RadarLine.facts` + vitals dates. `/api/cron/po-monitor` posts the written brief and falls back to the old deterministic blocks if the model is down. `briefingBlocks()` converts model `---` lines → real Slack dividers and fully-italic lines → dim context blocks. Brief is dated in **America/New_York** and the prompt pins the date (shopify_metrics syncs a UTC day ahead — "Sales thru" can read one day ahead on evening dry runs; self-consistent at the 14:00 UTC schedule). **Iterate with `GET /api/cron/po-monitor?dry=1`** (CRON_SECRET) — returns the composed brief without posting or consuming the dedupe key. Do NOT trigger a real post off-schedule: dedupe key is the UTC date, so it suppresses that day's scheduled run. Verified twice against prod data via dry runs; first real post is the next 14:00 UTC cron.
